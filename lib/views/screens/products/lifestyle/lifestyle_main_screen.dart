...
class LifestyleMainPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    ref.listen<LifeStyleChangeNotifier>(lifestyleChangeNotifier,
        (previous, next) async {
      final newData = await next.initOrderResponse?['status'];
      if (newData == 201) {
        processProvider.processId = next.initOrderResponse["processId"];
        EasyLoading.dismiss();
      ...
      if (next.isLoading) {
        EasyLoading.show(
            maskType: EasyLoadingMaskType.clear, status: 'Creating order');
      }
    });
